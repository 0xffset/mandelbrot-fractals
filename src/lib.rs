...
impl Mandelbrot {
    ...
    fn calculate_pixel(&self, x: u32, y: u32) -> u32 {
        if self.samples <= 1 {
            ...
            let cy = self.pos_y + (y as f64 / self.height as f64 - 0.5) * self.size * (self.height as f64 / self.width as f64);
        ...
        for sx in 0..self.samples {
            for sy in 0..self.samples {
                let cx = self.pos_x + ((x as f64 + step * (sx as f64 + 1.0)) / self.width as f64 - 0.5) * self.size;
                let cy = self.pos_y + ((y as f64 + step * (sy as f64 + 1.0)) / self.height as f64 - 0.5) * self.size * (self.height as f64 / self.width as f64);
    ...
    fn map_color(&self, iterations: u32) -> [u8; 4] {
        ...
        match self.paint_mode {
            ...
            PaintMode::HSL3 => {
                // Smooth coloring
                let log_zn = 0.0;
                let nu = (iterations as f64 + 1.0 - log_zn.log2().log2()) / self.max_iterations as f64;
    ...
    pub async fn render_parallel(&mut self, chunks: usize) -> Result<(), JsValue> {
        ...
        // Split work into chunks for better UI responsiveness
        ...
        for chunk in 0..chunks as u32 {
            ...
            wasm_bindgen_futures::JsFuture::from(js_sys::Promise::resolve(&JsValue::from(0))).await?;
        }
